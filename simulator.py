...
# --- 3. وظيفة دعم اللغة العربية (RTL) ---
def format_bidi_text(text, lang):
    if lang == 'Arabic':
        return f'<div style="direction: rtl; unicode-bidi: embed; text-align: right;">{text}</div>'
    return text

# --- 4. تعريف النصوص الديناميكية للواجهة (مع تعديل التعليمات) ---
def get_texts(lang):
    # التعليمات التي طلبتها بالضبط
    ...
    if lang == 'English':
        return {
            ...
            'instructions': f"{speak_instruction_english}", # تم التعديل لتكون الجملة المطلوبة فقط
            ...
            'chat_input_prompt': "Type your question or concern here...",
            'thinking_spinner': "AI Educator is counselling...",
            'transcribed_text': "Transcribed Text:",
            'error': "An API error occurred:",
            'accent_options': ["Empathetic & Clear", "Formal & Technical", "Quick & Direct"],
            'gemini_model': "gemini-1.5-flash", # موديل ثابت ومستقر
            'tts_lang_code': 'en',
            'stt_lang_code': 'en',
            'feedback_button': "Generate AI Educator Self-Assessment Report 📄",
            'feedback_spinner': "AI is analyzing performance and generating report...",
            'feedback_title': f"AI Educator Self-Assessment Report: {PRODUCT_NAME}",
            'welcome_msg': f"Hello, welcome. I am your Patient Educator. I understand you've been prescribed **{PRODUCT_NAME}** for Nausea and Vomiting of Pregnancy (NVP). Before we start, can you please tell me your name?"
        }
    else: # Arabic
        return {
            'title': f"محاكاة إرشاد المريض: **الذكاء الاصطناعي كمعلم** لدواء **{PRODUCT_NAME}**",
            'subheader': "لعب دور المريض لتقييم أداء المعلم",
            'instructions': f"{speak_instruction_arabic}", # تم التعديل لتكون الجملة المطلوبة فقط
            ...
            'feedback_spinner': "الذكاء الاصطناعي يحلل أداءه الإرشادي ويولد التقرير المهيكل...",
...
# --- 5. التحكم في الشريط الجانبي (Sidebar) ---
selected_language = st.sidebar.selectbox("1. اختيار اللغة", options=["Arabic", "English"], index=0, key='language_selector')
...
with main_container:
    ...
    # صندوق التعليمات (Expander)
    with st.expander("Session Instructions", expanded=True):
        st.markdown(texts['instructions'])

# --- 7. منطق النظام والذكاء الاصطناعي ---
def get_system_instruction(lang, accent, welcome_msg):
    if lang == 'English':
        educator_persona = f"You are a highly skilled, knowledgeable, and **{accent}** Patient Educator. The user is a pregnant patient named Sarah. Respond ONLY in English."
    else:
    ...
    2. Wait for name, then proceed to explain dosage and safety.
    """

current_system_instruction = get_system_instruction(selected_language, selected_accent, texts['welcome_msg'])
current_state_key = f"{selected_language}_{selected_accent}_{PRODUCT_NAME}_sim"
...
# --- 8. وظيفة التقييم الذاتي ---
def get_feedback():
    history = chat_session.get_history()
    if len(history) < 3:
        st.session_state['feedback'] = "يجب إكمال جزء من المحادثة أولاً."
        return

    conversation_log = "\n".join([f"{msg.role}: {msg.parts[0].text}" for msg in history if msg.role != 'system'])
    feedback_prompt = f"Analyze this conversation where you acted as an educator for {PRODUCT_NAME}. Provide a structured self-assessment in {selected_language}:\n{conversation_log}"
    
    with st.spinner(texts['feedback_spinner']):
        try:
            response = client.models.generate_content(model=texts['gemini_model'], contents=feedback_prompt)
            st.session_state['feedback'] = response.text
        except Exception as e:
            st.error(f"Error: {e}")

# --- 9. منطقة إدخال المستخدم (الميكروفون والكتابة) ---
...
with input_container:
    ...
    with col2:
        written_text = st.text_input(texts['chat_input_prompt'], label_visibility="collapsed", key='chat_patient')
        
    user_input = spoken_text if spoken_text else written_text

# --- 10. عرض الرسائل ومعالجة الردود ---
...
if user_input:
    ...
    with st.spinner(texts['thinking_spinner']):
        try:
            ...
            text_to_speech(text=ai_response, language=texts['tts_lang_code'], key=f"tts_{hash(ai_response)}")
        except Exception as e:
            st.error(f"Error: {e}")
...
# --- 11. قسم التقييم النهائي ---
st.markdown("---")
if st.button(texts['feedback_button'], use_container_width=True):
    get_feedback()
